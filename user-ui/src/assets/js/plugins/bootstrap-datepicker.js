...
(function (factory) {
  ...
  var DateArray = (function () {
    var extras = {
      ...
      contains: function (d) {
        ...
        {
          if (0 <= this[i].valueOf() - val && this[i].valueOf() - val < 1000
            * 60 * 60 * 24) {
            return i;
          }
        }
  ...
  var Datepicker = function (element, options) {
    ...
    this.component = this.element.hasClass('date') ? this.element.find(
      '.add-on, .input-group-addon, .btn') : false;
  ...
  Datepicker.prototype = {
    ...
    _process_options: function (opts) {
      ...
      o.startView = Math.max(this.o.minViewMode,
    ...
    place: function () {
      ...
      var calendarWidth = this.picker.outerWidth(),
        ...
        scrollTop = this.o.container === 'body' ? $(document).scrollTop()
          : container.scrollTop(),
    ...
    update: function () {
      ...
      if (fromArgs) {
        ...
        if (typeof this.o.format === 'string') {
          if ((String(this.element[0].value).length === String(
            this.o.format).length) && (String(oldDates) !== String(
            this.dates))) {
            this._trigger('changeDate');
          }
          this.element.change();
    ...
    getClassNames: function (date) {
      ...
      if (date.getUTCFullYear() < year || (date.getUTCFullYear() === year
        ...
        cls.push('old');
      } else if (date.getUTCFullYear() > year || (date.getUTCFullYear() === year
        && date.getUTCMonth() > month)) {
        cls.push('new');
      }
    ...
    _fill_yearsView: function (selector, cssClass, factor, year, startYear,
      ...
      for (var currVal = startVal - step; currVal <= endVal + step;
        currVal += step) {
        classes = [cssClass];
        tooltip = null;
    ...
    fill: function () {
      ...
      while (prevMonth.valueOf() < nextMonth) {
        ...
        html.push('<td class="' + clsName.join(' ') + '"' + (tooltip
          ? ' title="' + tooltip + '"' : '') + (this.o.dateCells
          ? ' data-date="' + prevMonth.getTime().toString() + '"' : '')
          + '><div>' + prevMonth.getUTCDate() + '</div></td>');
        tooltip = null;
    ...
    click: function (e) {
      ...
      if (target.hasClass('datepicker-switch') && this.viewMode
        !== this.o.maxViewMode) {
  ...
  var datepickerPlugin = function (option) {
    ...
    if (this.length > 1) {
      throw new Error('Using only allowed for the collection of a single element ('
        + option + ' function)');
    } else {
  ...
  var DPGlobal = {
    ...
    parseFormat: function (format) {
      if (typeof format.toValue === 'function' && typeof format.toDisplay
        === 'function') {
        return format;
      }
